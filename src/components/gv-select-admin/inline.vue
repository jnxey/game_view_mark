  ...
  >
    ...
    >
      <div class="flex-row align-center">
        ...
        <span class="ml-12">{{ item.account }}</span>
      </div>
    </el-option>
    <template #loading>
...
<script>
export default { name: 'gv-select-admin-inline' };
</script>
<script setup>
...
const props = defineProps({
  ...
  placeholder: { type: String, default: $t('components.gv_selecte_admin.placeholder_search') },
...
const remoteMethod = async (query) => {
  ...
  loading.value = true;
  const [res] = await toAwait(adminBaseLists({ page_type: QUERY_LIST_TYPE.pager, page_no: 1, page_size: 5, account: query }));
  loading.value = false;
  isLoaded.value = true;
...
const handleBlur = async () => {
  ...
  const key = list.findIndex((val) => val.account === cacheQuery.value);
  if (checkIndexArray(key)) emit('update:modelValue', list[key]?.id);
  clearCacheQuery();
};
...
watch(
  ...
  async () => {
    ...
    const [res] = await toAwait(adminBaseLists({ page_type: QUERY_LIST_TYPE.pager, page_no: 1, page_size: 5, id: content.value }));
    tableData.value = res.lists;
  },
  { immediate: true }
);
