<template>
  <div class="config-wrap">
    ...
    <div class="config-title">
      <div class="title flex-1 fz-16 fw-bold text-main">{{ $t('user.list.label_washing_title') }}</div>
    </div>
    <!--    -->
    <div class="config-content">
      <div class="g-table">
        ...
        <div class="g-table-item title">{{ $t('user.list.label_washing_sync') }}</div>
      </div>
            ...
            <div class="g-table-item">
              <template v-if="configList[getConfigIndex(game.id, currency.id)]">
                <gv-input-amount-placeholder
                  ...
                  placeholder=""
                  class="w-full"
                  :data-command="GAME_CONFIG_MARK.washing"
                >
...
const handleSubmit = async (loading = true) => {
  const params = { user_id: userInfo.value?.id, attr_name: 'washing', data: configList };
