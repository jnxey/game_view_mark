<template>
  <gv-popup ref="popupRef" :title="popupTitle" :async="true" width="550px" @confirm="handleSubmit" @close="handleClose">
    <el-form class="ls-form" ref="formRef" :rules="rules" :model="formData" label-width="100px" @submit.prevent>
      <el-form-item :label="$t('user.type.name')" prop="name">
        <gv-input-language
          custom-class="ls-input"
          v-model="formData.name"
          :placeholder="$t('user.type.placeholder_name')"
          clearable
          :type="GV_INPUT_LANGUAGE_TYPE.user_type"
        />
      </el-form-item>
    </el-form>
  </gv-popup>
...
<script setup>
import { userTypeAdd, userTypeEdit } from '@/api/user/type';
...
const popupTitle = computed(() => (mode.value === OPERATE_TYPE.update ? $t('user.type.title_edit') : $t('user.type.title_add')));

const formTemp = { id: null, name: '' };
const formData = reactive({ ...formTemp });

const rules = {
  name: [{ required: true, message: $t('user.type.field_check_name'), trigger: ['blur'] }]
};
...
const handleSubmit = async () => {
  ...
  layer.loading();
  mode.value === OPERATE_TYPE.update ? await userTypeEdit(formData) : await userTypeAdd(formData);
  layer.closeLoading();
