...
import { calculateStayDays, deepClone, generateUUID, isEmpty, syncObjectData } from '@/tools';
...
import { hotelRoomRecordSet } from '@/api/external/hotel-room-record';
...
const popupTitle = computed(() => $t('external.hotel_room_record.title_edit'));
...
const open = async (row) => {
  ...
  const guests = deepClone(row.client ?? []).map((item, index) => {
    if (!index) {
      item.room_rates_true = row.room_rates_true;
      item.deposit = row.deposit;
    }
    item.UNIKEY = generateUUID();
    return item;
  });
  syncObjectData(formData, {
    ...formTemp,
    record_id: row.id,
    hotel_management_id: row.hotel_management_id,
...
const handleSubmit = async () => {
  ...
  await hotelRoomRecordSet(params);
  layer.closeLoading();
