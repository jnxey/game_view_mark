  ...
  <template v-for="(t1, x) in part.cols" :key="x">
    <template v-for="(t2, y) in part.rows" :key="getLevel2Key(x, y)">
      <div v-if="roadList[x] && roadList[x]?.[y]" class="col-box" :style="getPositionBySize(x, y, part.boxSize)">
        <!--      正常显示        -->
        <div
          class="road-item border"
          :style="{ borderColor: roadList[x][y].color, backgroundColor: roadList[x][y].color, color: roadList[x][y].color }"
        />
      </div>
    </template>
  </template>
  <canvas id="part6-canvas" class="part-canvas" :width="part.width" :height="part.height" />
</template>
<script setup>
