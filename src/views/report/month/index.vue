<template>
  <div class="page-wrap">
    <gv-table
      ref="tableRef"
      table-key="report_month-list"
      :request="reportMonthLists"
      row-key="UID"
      :operate-width="160"
      show-summary
      :handle-summary="useSummary('month')"
      :columns="columns"
    ...
    >
      <template #search-box="{ query, search, reset }">
        ...
        <gv-daterange-picker
          ...
          type="monthrange"
          :show-format="getTimeFormat('month')"
          v-place
          :clearable="false"
...
import { reportMonthDetail, reportMonthLists } from '@/api/report/month';
...
const {
  ...
  doubleExec
} = useTimeRange(() => tableRef.value, { defActive: TIME_RANGE_TYPE.thisMonth, defFilterTime: rangeTimeType.create_time }, true);
...
const columns = shallowRef([
  { title: $t('report.table_col.date'), dataIndex: 'month', minWidth: 120, click: (row) => handleBetDetails(row), subHide: true },
...
const { splitColumns, isMerge, expandValue, extendWidth, expandCache, refreshIsMerge, refreshSplitColumns, toggleRowExpansion, refreshExpandChange } =
  useCurrencyExtend(() => tableRef.value, reportMonthDetail, { key: 'data_month', value: 'month' });
