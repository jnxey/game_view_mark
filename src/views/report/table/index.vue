<template>
  <div class="page-wrap">
    <gv-table
      ref="tableRef"
      table-key="report_table-list"
      :request="reportTableLists"
      row-key="UID"
      :operate-width="160"
      :columns="columns"
      :default-query="defaultQuery"
      show-summary
      :handle-summary="useSummary('table_name')"
      delay-request
    ...
    <report-table-details ref="reportTableDetailsRef" />
  </div>
</template>
...
import { reportTableDetail, reportTableLists } from '@/api/report/table';
...
import ReportTableDetails from '@/views/report/table/_components/report-table-details.vue';
import { reportUserLists } from '@/api/report/user';
import { checkPerm } from '@/directives/perms';
import { gameKindLists } from '@/api/game/kind';
import { permsMap } from '@/api/perms';
import { useSummary } from '@/hooks/summary';
import { $t } from '@/lang/i18n';
import { EXPAND_TYPE, useCurrencyExtend } from '@/views/report/_hooks/column-extend';
...
const reportTableDetailsRef = shallowRef();
...
const columns = shallowRef([
  {
    ...
    minWidth: 100,
    click: checkPerm([reportUserLists.perm]) ? (row) => handleDetails(row) : null,
    subHide: true
  },
...
const { splitColumns, isMerge, expandValue, extendWidth, expandCache, refreshIsMerge, refreshSplitColumns, toggleRowExpansion, refreshExpandChange } =
  useCurrencyExtend(() => tableRef.value, reportTableDetail, { key: 'table_id', value: 'table_id' });
...
const handleDetails = async (row) => {
  ...
  reportTableDetailsRef.value?.open(row, query, timeRangeActive.value);
};
