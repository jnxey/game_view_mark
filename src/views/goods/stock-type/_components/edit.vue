<template>
  <gv-popup ref="popupRef" :title="popupTitle" :async="true" width="550px" @confirm="handleSubmit" @close="handleClose">
    <el-form class="ls-form" ref="formRef" :rules="rules" :model="formData" label-width="100px" @submit.prevent>
      <el-form-item :label="$t('goods.stock_type.name')" prop="name">
        <el-input v-model="formData.name" :placeholder="$t('goods.stock_type.placeholder_name')" clearable />
      </el-form-item>
    </el-form>
  </gv-popup>
</template>
<script setup>
import { stockTypeSet } from '@/api/goods/stock-type';
...
const popupTitle = computed(() => (mode.value === OPERATE_TYPE.update ? $t('goods.stock_type.title_edit') : $t('goods.stock_type.title_add')));
...
const rules = {
  name: [{ required: true, message: $t('goods.stock_type.field_check_name'), trigger: ['blur'] }]
};
