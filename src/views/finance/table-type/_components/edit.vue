<template>
  <gv-popup ref="popupRef" :title="popupTitle" :async="true" width="550px" @confirm="handleSubmit" @close="handleClose">
    <el-form class="ls-form" ref="formRef" :rules="rules" :model="formData" label-width="120px" @submit.prevent>
      <el-form-item :label="$t('finance.table_type.name')" prop="name">
        <gv-input-language
          custom-class="ls-input"
          v-model="formData.name"
          :placeholder="$t('finance.table_type.placeholder_name')"
          clearable
          :type="GV_INPUT_LANGUAGE_TYPE.table_type_name"
        />
      </el-form-item>
    </el-form>
  </gv-popup>
...
<script setup>
import { financeTableTypeSet } from '@/api/finance/table-type';
...
const popupTitle = computed(() => (mode.value === OPERATE_TYPE.update ? $t('finance.table_type.title_edit') : $t('finance.table_type.title_add')));
...
const handleSubmit = async () => {
  ...
  layer.loading();
  await financeTableTypeSet(formData);
  layer.closeLoading();
