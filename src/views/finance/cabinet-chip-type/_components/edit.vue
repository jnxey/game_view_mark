<template>
  <gv-popup ref="popupRef" :title="popupTitle" :async="true" width="550px" @confirm="handleSubmit" @close="handleClose">
    <el-form class="ls-form" ref="formRef" :rules="rules" :model="formData" label-width="120px">
      <el-form-item :label="$t('finance.cabinet_chip_type.name')" prop="name">
        <gv-input-language
          custom-class="ls-input"
          v-model="formData.name"
          :placeholder="$t('finance.cabinet_chip_type.placeholder_name')"
          clearable
          :type="GV_INPUT_LANGUAGE_TYPE.cabinet_chip_type_name"
        />
      </el-form-item>
    </el-form>
  </gv-popup>
...
<script setup>
import { financeCabinetChipTypeSet } from '@/api/finance/cabinet-chip-type';
...
const popupTitle = computed(() =>
  mode.value === OPERATE_TYPE.update ? $t('finance.cabinet_chip_type.title_edit') : $t('finance.cabinet_chip_type.title_add')
);
...
const rules = {
  name: [{ required: true, message: $t('finance.cabinet_chip_type.field_check_name'), trigger: ['blur'] }]
};
...
const handleSubmit = async () => {
  ...
  layer.loading();
  await financeCabinetChipTypeSet(formData);
  layer.closeLoading();
