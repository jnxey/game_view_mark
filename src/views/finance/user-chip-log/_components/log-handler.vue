  ...
  <div class="page-wrap">
    ...
    >
      ...
      <template #handler-left>
        <div class="handler-left-wrap">
          <template v-if="isChipBalance">
            <el-button id="chip-change-summary-trigger" type="primary" @click="showSummaryInfo">
              <template #icon>
                <gv-icon name="local-icon-statics" />
              </template>
              {{ $t('finance.user_chip_log.summary_title') }}
            </el-button>
          </template>
          <template v-if="isRemitCode && !!remitCount">
            <span class="mr-8">{{ $t('finance.user_chip_log.remit_count_title') }}:</span>
            <template v-for="item in optionsData.currencyBase ?? []">
              <span v-if="!!remitCount[item.id]" class="mr-8 fs-italic" :style="{ color: item.color }">
                {{ item.icon }}{{ remitCount[item.id].total_amount }}
              </span>
            </template>
          </template>
          <chip-change-summary ref="chipChangeSummaryRef" :currency-config="optionsData.currencyConfig" />
        </div>
      </template>
...
<script setup>
import { computed, nextTick, onActivated, onBeforeMount, ref, shallowRef, watch } from 'vue';
import GvTable from '@/components/gv-table/index.vue';
import { backTab } from '@/store/tabs';
import { financeActionsSummary, financeRemittanceList, financeUserLog } from '@/api/finance/user-chip';
...
import ChipChangeSummary from '@/views/finance/user-chip-log/_components/chip-change-summary.vue';
import GvIcon from '@/components/gv-icon/index.vue';
import { currencyBaseLists } from '@/api/currency/base';
import { CURRENCY_TYPE_DICT, IS_MEMBER } from '@/values';

const remitCount = ref(null);
...
const isChipBalance = computed(() => route.query.sub_type === BALANCE_TYPE.chip_balance.user_unique);
const isRemitCode = computed(() => route.query.sub_type === BALANCE_TYPE.cabinet_remit.user_unique);
const isPointAmount = computed(() => route.query.sub_type === BALANCE_TYPE.point_amount.user_unique);
...
const actionList = computed(() => {
  ...
  let action = [];
  if (query.sub_type === BALANCE_TYPE.chip_balance.user_unique) return FINANCE_ACTION.map((item) => ({ ...item, field: 'action_from' }));
  if (query.sub_type === BALANCE_TYPE.code_washing.user_unique) action = [30, 31];
  if (query.sub_type === BALANCE_TYPE.deposit_amount.user_unique) action = [22, 23];
  if (query.sub_type === BALANCE_TYPE.cabinet_buy.user_unique) action = [20, 21];
  if (query.sub_type === BALANCE_TYPE.sign_amount.user_unique) action = [24, 25];
  if (query.sub_type === BALANCE_TYPE.point_amount.user_unique) action = [60, 61];
  if (query.sub_type === BALANCE_TYPE.cabinet_remit.user_unique) action = [70, 71];
  if (query.sub_type === BALANCE_TYPE.chip_transfer.user_unique) action = [27, 41];
  action = action.map((id) => {
    return { ...financeActionMap[id], field: 'action_from' };
  });
  if (query.sub_type === BALANCE_TYPE.chip_transfer.user_unique) {
    action = [
      { name: $t('finance.user_chip_log.transfer_inc'), value: changeTypeMap.inc, field: 'change_type' },
      { name: $t('finance.user_chip_log.transfer_dec'), value: changeTypeMap.dec, field: 'change_type' },
      ...action
    ];
  }
  return [...action];
...
const { optionsData, refresh } = useDictOptions(
  {
    currencyConfig: { api: currencyConfigLists, params: { virtual_status: null } },
    currencyBase: { api: currencyBaseLists, params: { virtual_status: null } }
  },
  true
);
...
const columns = computed(() => {
  return [
    ...
    { title: $t('common.log_before_value'), dataIndex: 'before_value', minWidth: 150, hide: isPointAmount.value },
    { title: $t('log.change_value'), dataIndex: 'left_amount', minWidth: 150 },
    { title: $t('common.log_after_value'), dataIndex: 'after_value', minWidth: 150, hide: isPointAmount.value },
    {
      title: $t('log.action_from'),
      dataIndex: 'action_from',
      minWidth: 100,
      func_html: true,
      func: (_, val) => {
        const cMap = GLOBAL_DICT.value.currency_config.map;
        const currencyType = cMap[val.currency_config_id]?.type;
        const name = !!financeActionMap[val.action_from]?.getName
          ? financeActionMap[val.action_from]?.getName(currencyType)
          : financeActionMap[val.action_from]?.name;
        return `<span style="color:${financeActionMap[val.action_from]?.color};">${name}</span>`;
      }
    },
...
// 获取汇款总计
const getRemitCount = async () => {
  if (!isRemitCode.value) return (remitCount.value = null);
  const res = await financeRemittanceList();
  remitCount.value = res && !!res.length ? mappingArrayToObject(res, 'currency_id') : null;
};
