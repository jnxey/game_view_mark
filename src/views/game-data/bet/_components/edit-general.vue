...
const open = async (info) => {
  ...
  const kindInfo = gameKindMap.value[info.game_kind_id] ?? {};
  ...
  if (isTop(kindInfo.is_top) && kindInfo.sub_calc === GAME_KIND_CALC.self) {
    formData.gameKindMainId = info.game_kind_id;
  } else {
    formData.gameKindMainId = kindInfo.sub_main_id;
    formData.gameKindSubId = info.game_kind_id;
  }
  popupRef.value?.open();
};
...
const handleSubmit = async () => {
  ...
  const { id, open_id, user_id, is_nn_win, currency_config_id, amount, round_number, bonus, bonus_id, bonus_profit } = formData;
  const params = {
    id,
    open_id,
    user_id,
